...
namespace ERP.EntityFrameworkCore.Repositories
{
    public static class EFExtensions
    {
        ...
        public class SprocResults
        {
            ...
            public static IList<T> MapToList<T>(DbDataReader dr) where T : new()
            {
                ...
                while (dr.Read())
                {
                    ...
                    try
                    {
                        ...
                        foreach (var prop in props)
                        {
                            ...
                            {
                                prop.SetValue(obj, val == DBNull.Value ? "" : val.ToString());

                            }
                            else if (prop.PropertyType == typeof(DateTime))
                            {
                                prop.SetValue(obj, val == DBNull.Value ? DateTime.MinValue : Convert.ToDateTime(val));
                    ...
                    {
                        throw new UserFriendlyException($"Error In Mapping \n The Property {upperName} has an Error Please Check Its Data Type \n {e.Message}");
                    }
                }
            ...
            bool manageConnection = true)
        {
            ...
            using (command)
            {
                ...
                try
                {
                    ...
                    //if (command.Connection.Database.CurrentTransaction != null)
                    //{
