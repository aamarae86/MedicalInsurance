...
$(function () {
    ...
    //debugger;
    //document.querySelectorAll(`.form-line input[disabled='true']`)
    //    .forEach(el => {
    //        if (!el.value.length)
    //            el.nextSibling.nextSibling.style.background = "transparent"  
    //    })
...
$.MyAdmin.leftSideBar = {
    ...
    setMenuHeight: function (isFirstTime) {
        if (typeof $.fn.slimScroll != 'undefined') {
            ...
            //$('.navbar').innerHeight()
            //console.log(`Window height ${$(window).height()} and logo ${$('.side-logo').outerHeight() + 60} and finally slimScroll is ${height}`)
            var $el = $('.list');

            $el.slimscroll({
                position: `${$('html').attr('lang') === 'ar-EG' ? 'left' : 'right'}`,
                height: height + "px",
